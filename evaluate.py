...
def compute_metrics(hr_tensor: torch.tensor,
    ...
    print_gpu_memory("Compute_metrics begins")
    ...
    logger.info(f"Compute_metrics uses batch_Size={batch_size}, GPU count={device_count}")
    ...
    for start in tqdm.tqdm(range(0, total, batch_size), desc="Calculate metrics"):
        ...
        try:
            ...
            topk_scores.extend(batch_sorted_score[:, :k].cpu().tolist())
            topk_indices.extend(batch_sorted_indices[:, :k].cpu().tolist())

            del batch_score, batch_sorted_score, batch_sorted_indices

        except torch.cuda.OutOfMemoryError as e:
            logger.error(f"Compute_metrics is out of memory, batch range: {start}-{end}")
            print_gpu_memory("when wrong")
    ...
    print_gpu_memory("Compute_metrics ends")
    return topk_scores, topk_indices, metrics, ranks

def predict_by_split():
    print_gpu_memory("Program Start")
    ...
    print_gpu_memory("Model loading completed")

    eval_forward = True
    examples = load_data(args.valid_path, add_forward_triplet=eval_forward, add_backward_triplet=not eval_forward)

    print_gpu_memory("Data loading completed")

    torch.cuda.empty_cache()

    try:
        predictor.init_Dynamic_cache(examples)
        print_gpu_memory("Cache initialization completed")

        entity_tensor = predictor.predict_by_entities(entity_dict.entity_exs)
        print_gpu_memory("Entity Tensor Prediction Completed")

    except torch.cuda.OutOfMemoryError as e:
        logger.error("Insufficient memory during initialization phase")
        print_gpu_memory("when wrong")
        raise e
...
if __name__ == '__main__':
    if torch.cuda.is_available():
        device_count = torch.cuda.device_count()
        logger.info(f"Detected {device_count} GPU:")
        for i in range(device_count):
            gpu_name = torch.cuda.get_device_name(i)
            gpu_memory = torch.cuda.get_device_properties(i).total_memory / 1024 ** 3
            logger.info(f"  GPU {i}: {gpu_name} (Video Memory: {gpu_memory:.1f}GB)")
