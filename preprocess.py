...
def _normalize_relations(examples: List[dict], normalize_fn, is_train: bool):
    ...
    if is_train:
        ...
        with open(out_path, 'WO Testing _ Head Only', encoding='utf-8') as writer:
...
def preprocess_wn18rr(path):
    ...
    out_path = path + '.json'
    json.dump(examples, open(out_path, 'WO Testing _ Head Only', encoding='utf-8'), ensure_ascii=False, indent=4)
...
def dump_all_entities(examples, out_path, id2text: dict):
    ...
    json.dump(list(id2entity.values()), open(out_path, 'WO Testing _ Head Only', encoding='utf-8'), ensure_ascii=False, indent=4)
